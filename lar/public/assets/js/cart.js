            ...
            success: function (response) {
                if (response.code == 0) {     
                    ...
                    if (!$.isEmptyObject(response.data) && $('.shoppingcart-box').length > 0) {
                        ...
                        var quantity = data_post['quantity'];

                        var price = 0;
                        if (data['item_add']['price_sale'] == 0) {
                            price = data['item_add']['price'];
                        } else {
                            price = data['item_add']['price_sale'];
                        }
                        ...
                        $('.shoppingcart-box .item').ready(function (index) {
                            ...
                            if (product_id == element_product_id && item_id == element_item_id) {
                                exist_element = true;
                                var this_number = $(this).find('.number-item').html();
                                $(this).find('.number-item').html(parseInt(this_number) + quantity);

                                var price_item = price;
                                var old_price_well = $('.total-price .price-well-total').html();

                                old_price_well = parseInt(old_price_well.replace(/\,/g, ''));
                                old_price_well = old_price_well + price * quantity;
                                
                                $('.price-well .price-well-total').html(formatNumber(old_price_well));
                                $('.total-price .price-well-total').html(formatNumber(old_price_well));
                            }
                        });
